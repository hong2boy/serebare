  ...
  var $container = function(){/*
    ...
    */}.toString().slice(14,-3);
    ...
    zoom = function ($piProductNo, $piCategoryNo, $piDisplayGroup) {
      ...
      $('#modalContainer').center().show(function() {

        var iContents = $('#modalContainer').find('#modalContent').contents().find('body');


        var prdZoomScope = iContents.find('th[scope="row"]');
        prdZoomScope.each(function() {
          var prdZoomScopeText = $(this).find('span').html();
          if (prdZoomScopeText == "비디오")
          {
            $(this).parent("tr").hide();
          }
        });
